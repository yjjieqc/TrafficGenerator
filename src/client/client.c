...
//is client send request in a fixed interval, and in a fixed flow size
bool is_fixed_interval = 0;
//set the throughput log time interval
unsigned int t_throughput_us = 10000;
//set the throughput log file prefix
char pre_th_log[100] = {0};
...
void print_usage(char *program)
{
    ...
    printf("-fi <fixed interval> each client sends request in the fixed interval us/sec\n");
    printf("-ot <pre_th_log> the prefix of throughput log file \n");
    printf("-tt <t_throughput_us > log the throughput file in us granularity \n");
...
void read_args(int argc, char *argv[])
{
    ...
    while (i < argc)
    {
        ...
        else if (strlen(argv[i]) == 3 && strcmp(argv[i], "-fi") == 0)
        {
            if(i+1 < argc)
            {
                is_fixed_interval = 1;
            ...
            {
                printf("Cannot read the fixed interval\n");
                print_usage(argv[0]);
                exit(EXIT_FAILURE);
            }
        }
        else if (strlen(argv[i]) == 3 && strcmp(argv[i], "-ot") == 0)
        {
            if (i+1 < argc && strlen(argv[i+1]) < sizeof(pre_th_log))
            {
                sprintf(pre_th_log, "%s", argv[i+1]);
                i += 2;
            }
            else
            {
                printf("Cannot read prefix throughput log file name\n");
                print_usage(argv[0]);
                exit(EXIT_FAILURE);
            }
        }
        else if (strlen(argv[i]) == 3 && strcmp(argv[i], "-tt") == 0)
        {
            if (i+1 < argc)
            {
                t_throughput_us = atof(argv[i+1]);
                if (t_throughput_us < 0)
                {
                    printf("the illegal throughput log time \n");
                    exit(EXIT_FAILURE);
                }
                i += 2;
            }
            ...
            {
                printf("Cannot resolve the throughput log time \n");
                print_usage(argv[0]);
                exit(EXIT_FAILURE);
            }
        }
        ...
        else if (!strcmp(key, "req_size_dist"))
        {
            ...
            if (verbose_mode)
            {
                ...
                if (is_fixed_interval)
                    printf("Fixed request size: %.2f bytes\n", gen_fixed_length(req_size_dist));
                else
...
void set_req_variables()
{
    ...
    {
        if (is_fixed_interval)
            period_us = gen_fixed_length(req_size_dist) * 8 / load / TG_GOODPUT_RATIO;
        else
            period_us = avg_cdf(req_size_dist) * 8 / load / TG_GOODPUT_RATIO;
        printf("period_us is %d \n", period_us);
        if (period_us <= 0)
        {
            cleanup();
    ...
    {
        if(is_fixed_interval)
            req_size[i] = gen_fixed_length(req_size_dist);     // fixed flow size
        else
        ...
        if(is_fixed_interval)
            req_sleep_us[i] = uniform_gen_interval(period_us);
        else
...
void *listen_connection(void *ptr)
{
    ...
    char throughput_log_name[100] = {0};
    int n;
    ...
    while (true)
    {
        ...
        {
            n = snprintf(throughput_log_name, sizeof(throughput_log_name), "%s/%s-%d-%d", pre_th_log, node->list->ip, node->list->port, flow.id);
            if (n >= sizeof(throughput_log_name))
            {
                perror(" snprintf buffer overflow");
                exit(EXIT_FAILURE);
            }
            if (ex_read_exact(node->sockfd, read_buf, flow.size, TG_MAX_READ, true, t_throughput_us, throughput_log_name) != flow.size)
            {
                perror("Throughput error: receive flow");
                break;
            }
        }
...
void print_statistic()
{
    ...
    unsigned long long flow_goodput_mbps;    /* per-flow goodput (Mbps) */
    unsigned int goodput_mbps; /* total goodput (Mbps) */
    unsigned int i = 0;
//    avoid unsighed int overflow
    unsigned long long tmp_goodput;
    FILE *fd = NULL;
    ...
    for (i = 0; i < req_total_num; i++)
    {
        ...
        if (fct_us > 0){
            tmp_goodput = req_size[i];
            tmp_goodput *= 8;
            flow_goodput_mbps = tmp_goodput / fct_us;
        }
        else
            flow_goodput_mbps = 0;

        /* size (bytes), FCT(us), DSCP, sending rate (Mbps), goodput (Mbps) */
        fprintf(fd, "%u %llu %u %u %llu\n", req_size[i], fct_us, req_dscp[i], req_rate[i], flow_goodput_mbps);
    }

    fclose(fd);
    goodput_mbps = req_size_total * 8 / duration_us;
//    printf("the flow_goodput_mbps is %llu, the fct_is is %llu; the actual goodput is %u, the duration is %llu \n",flow_goodput_mbps,fct_us,goodput_mbps,duration_us);
